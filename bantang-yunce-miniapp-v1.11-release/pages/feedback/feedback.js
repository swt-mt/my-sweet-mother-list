...
Page({
  data: {
    ...
    sourceLabel: '首页',
    itemLabel: '',
    form: { type: 'feature', source: 'index', itemName: '', content: '', contact: '' },
    images: [],
    feedbackRecords: [],
    source: 'index',
    itemId: '',
  ...
  onLoad(options = {}) {
    ...
    const itemLabel = options.itemId ? this.resolveItemName(options.itemId) : ''
    this.setData({
      source,
      itemId: options.itemId || '',
      focusFeedbackId: options.feedbackId || '',
      sourceLabel,
      itemLabel,
      presetName: options.itemId ? itemLabel : '',
      'form.source': source,
      'form.itemName': itemLabel,
      auth: loadAuth()
    })
    this.refreshRecords()
    trackEvent('page_view', { page: 'feedback', source })
  },
  onShow() { this.setData({ auth: loadAuth() }); this.refreshRecords() },
  refreshRecords() {
  ...
  onItemNameInput(e) { this.setData({ 'form.itemName': e.detail.value, itemLabel: e.detail.value }) },
  onContentInput(e) { this.setData({ 'form.content': e.detail.value }) },
  onContactInput(e) { this.setData({ 'form.contact': e.detail.value }) },
  chooseImage() {
    if (!requireLogin('添加反馈图片', '/pages/auth/auth')) return
    wx.chooseImage({
  ...
  submit() {
    ...
    this.setData({ form: { type: 'feature', source: this.data.source || 'index', itemName: this.data.presetName || '', content: '', contact: '' }, images: [] })
    this.refreshRecords()
